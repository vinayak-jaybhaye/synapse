...
export const mediaApi = {
  generateAvatarUploadUrl: async (payload: Record<string, unknown>): Promise<UploadResponse> => {
    const response = await api.post<UploadResponse>("/users/@me/avatars/upload-url", payload);
    return response.data;
  },

  generateUserBannerUploadUrl: async (
  ...
  generateGuildIconUploadUrl: async (
    guildId: string,
    payload: Record<string, unknown>,
  ): Promise<UploadResponse> => {
  ...
  generateGuildBannerUploadUrl: async (
    guildId: string,
    payload: Record<string, unknown>,
  ): Promise<UploadResponse> => {
