...
export default function UserProfileCard({ profile, isLoading, onClose }: UserProfileCardProps) {
  ...
  if (isLoading) {
    return (
      <div className="w-[300px] max-w-[calc(100vw-32px)] bg-[#111214] border border-[#1e1f22] rounded-lg overflow-hidden shadow-xl flex flex-col animate-pulse">
  ...
  return (
    <div className="w-[300px] max-w-[calc(100vw-32px)] bg-[#111214] border border-[#1e1f22] rounded-lg overflow-hidden shadow-xl flex flex-col text-text-primary">
      {/* Banner */}
      <div
        ...
        style={
          profile.banner_key
            ? {
                backgroundImage: `url(${getMediaUrl(profile.banner_key)})`,
      ...
      <div className="px-4 pb-4">
        ...
        <div className="mb-3 min-w-0">
          ...
          <p className="text-sm text-text-muted truncate" title={profile.username}>
            @{profile.username}
          </p>
        </div>
        ...
        {profile.mutual_guilds !== undefined && profile.mutual_guilds > 0 && (
          <div className="mb-4">
            <p className="text-sm text-text-secondary">
              {profile.mutual_guilds} Mutual Server
              {profile.mutual_guilds !== 1 ? "s" : ""}
            </p>
          </div>
        )}
